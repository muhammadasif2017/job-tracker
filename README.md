...
See [docs/architecture.md](docs/architecture.md) for system context, data flow, module, auth, async pipeline and deployment diagrams. [docs/c4.md](docs/c4.md) has C4 context, container and component diagrams, and [docs/uml.md](docs/uml.md) has UML state and sequence diagrams for company enrichment status, extension job capture, resume upload and notification digests. A styled version of the data flow diagram lives at <https://claude.ai/artifact/XE3NMk5ubDKpuLh1LnMUXm> (private; needs the owner's claude.ai sign-in).
