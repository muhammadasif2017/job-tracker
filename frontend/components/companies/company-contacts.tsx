...
export function CompanyContacts({ companyId, contacts }: CompanyContactsProps) {
  ...
  function buildPayload(): CompanyContactPayload {
    return {
      name: form.name.trim(),
      role: form.role || null,
  ...
  const createMutation = useCreateCompanyContactMutation(companyId, resetForm);
  const updateMutation = useUpdateCompanyContactMutation(companyId, resetForm);
  const removeMutation = useRemoveCompanyContactMutation(companyId, () =>
