...
export interface ContactsPanelProps {
  ...
  updateMutation: UseMutationResult<
    unknown,
    unknown,
    { contactId: string; payload: ContactPayload }
  >;
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (editingId) {
      updateMutation.mutate(
        { contactId: editingId, payload: buildPayload() },
        { onSuccess: resetForm },
      );
    } else {
      createMutation.mutate(buildPayload(), { onSuccess: resetForm });
    }
  }
    ...
    >
      ...
      {formOpen && (
        <form
          ...
          className={cn(
            'flex flex-col gap-3 rounded-md border border-line p-3',
            nested ? 'mb-3' : 'mb-4',
          )}
        >
