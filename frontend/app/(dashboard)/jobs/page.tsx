...
import { JOB_STATUSES, STATUS_LABELS, type Job, type PaginatedJobs, type JobStatus } from '../../../types';
...
export default function JobsPage() {
  ...
  const openEdit = (job: Job) => { setEditJob(job); setFormOpen(true); };
  const closeForm = () => { setFormOpen(false); setEditJob(undefined); };
  ...
  const handleExport = async () => {
    try {
      ...
      const res = await api.get(`/jobs/export?${exportParams}`, { responseType: 'blob' });
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">{data?.meta.total ?? 0} applications tracked</p>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex rounded-lg border border-slate-300 dark:border-slate-700">
          <button onClick={() => setView('list')} className={`flex items-center gap-1.5 px-3 py-1.5 text-sm rounded-l-lg transition-colors ${view === 'list' ? 'bg-indigo-600 text-white' : 'text-slate-600 hover:bg-slate-50 dark:text-slate-400 dark:hover:bg-slate-800'}`}>
            <List className="h-3.5 w-3.5" /> List
          </button>
          <button onClick={() => setView('kanban')} className={`flex items-center gap-1.5 px-3 py-1.5 text-sm rounded-r-lg transition-colors ${view === 'kanban' ? 'bg-indigo-600 text-white' : 'text-slate-600 hover:bg-slate-50 dark:text-slate-400 dark:hover:bg-slate-800'}`}>
                ...
                ? (
                  <tr>
                    <td colSpan={6} className="py-16 text-center text-slate-400">
                      ...
                      <p className="mt-1 text-sm">Add your first application to get started.</p>
                    </td>
                  </tr>
                )
                : data?.data.map((job) => (
                  <tr key={job.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                    <td className="px-4 py-3 font-medium">
                      <Link href={`/jobs/${job.id}`} className="hover:text-indigo-600">{job.company}</Link>
                    </td>
                    <td className="px-4 py-3 text-slate-600 dark:text-slate-400">{job.position}</td>
                    <td className="px-4 py-3"><StatusBadge status={job.status} /></td>
                    <td className="px-4 py-3 text-slate-500 whitespace-nowrap">{formatDate(job.appliedAt)}</td>
                    <td className="px-4 py-3 text-slate-500">{job.location ?? '—'}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1 justify-end">
                        {job.url && (
                          <a href={job.url} target="_blank" rel="noopener noreferrer" className="rounded p-1.5 text-slate-400 hover:text-indigo-600">
                            <ExternalLink className="h-3.5 w-3.5" />
                          </a>
                        )}
                        <button onClick={() => openEdit(job)} className="rounded p-1.5 text-slate-400 hover:text-indigo-600">
                          <Pencil className="h-3.5 w-3.5" />
                        </button>
                        <button onClick={() => deleteMutation.mutate(job.id)} className="rounded p-1.5 text-slate-400 hover:text-red-600">
          ...
          {data && data.meta.totalPages > 1 && (
            <div className="flex items-center justify-between border-t px-4 py-3 text-sm text-slate-500">
              ...
              <div className="flex gap-2">
                ...
                <Button variant="secondary" size="sm" disabled={page === data.meta.totalPages} onClick={() => setPage(p => p + 1)}>Next</Button>
              </div>
            </div>
          )}
        </div>
