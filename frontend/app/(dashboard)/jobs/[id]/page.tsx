...
import { JOB_STATUSES, STATUS_LABELS, type Job, type JobEvent, type JobStatus } from '../../../../types';
...
function Timeline({ events }: { events: JobEvent[] }) {
  ...
  return (
    <div className="rounded-xl border bg-white p-6 dark:bg-slate-900 space-y-4">
      <h2 className="font-semibold text-sm uppercase tracking-wide text-slate-500">Timeline</h2>
...
export default function JobDetailPage() {
  ...
  const patchStatus = useMutation({
    mutationFn: (status: JobStatus) => api.patch(`/jobs/${id}`, { status }).then((r) => r.data),
    onSuccess: (updated) => {
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <Link href="/jobs" className="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-slate-900 dark:hover:text-slate-100">
          ...
          <div className="rounded-xl border bg-white p-6 dark:bg-slate-900 space-y-5">
            ...
            <div className="grid gap-4 sm:grid-cols-2 text-sm">
              <div>
                ...
                <select
                  value={job.status}
                  onChange={(e) => patchStatus.mutate(e.target.value as JobStatus)}
              ...
              <div>
                <p className="text-xs text-slate-400 uppercase tracking-wide mb-1">Applied</p>
                <p>{formatDate(job.appliedAt)}</p>
              </div>
              {job.nextInterviewAt && (
                <div>
                  <p className="text-xs text-slate-400 uppercase tracking-wide mb-1">Next Interview</p>
              ...
              {job.url && (
                <div>
                  <p className="text-xs text-slate-400 uppercase tracking-wide mb-1">Job Posting</p>
                  <a href={job.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-indigo-600 hover:underline">
            ...
            {job.notes && (
              <div>
                ...
                <p className="whitespace-pre-wrap rounded-lg bg-slate-50 p-3 text-sm dark:bg-slate-800">{job.notes}</p>
              </div>
            )}
          </div>
