...
Retry: `attempts: 2`, fixed 10s backoff. On failure the processor's `catch` block normally writes `status: FAILED` + a sanitized `errorMessage` (URLs stripped, capped at 200 chars) before rethrowing — the rethrow is what makes BullMQ actually retry. **Exception**: if extraction (`LlmService.extract` + the guard in §6) already succeeded before the failure — i.e. only a later step like the final DB write itself threw — the catch block salvages that data as a `COMPLETED` write instead, logs `enrichment_completed_after_late_failure`, and returns without rethrowing (no BullMQ retry — there's no point re-running a pipeline that already produced good data). If that salvage write also throws, it falls back to the original FAILED-write-and-rethrow path. See §10 for the reasoning.
...
`backend/src/modules/enrichment/services/llm.service.ts`. Model: `openai/gpt-oss-120b` via Groq (migrated 2026-08-17 — Groq deprecated `llama-3.3-70b-versatile` on 2026-08-16), forced tool-call (`tool_choice: 'required'`) against a fixed JSON schema (`extract_company_data`) — see ADR-002 for why tool-call over free-text parsing.
...
const guardThresholds = { address: 0.7, headquarters: 0.25 };
...
| `headquarters` | ≥25% of tokens | Same official-page text (**not** `location`) | `headquarters` has no official-only sourcing restriction in the prompt — legitimate values can come from third-party text (news, LinkedIn) — so a strict 70% bar would flag too much as unverified. Lowered from an initial 0.4 to 0.25 once the guard stopped discarding below-bar values (see above): with a reject now costing only a badge instead of hiding the field, the balance shifted further toward accepting. The job's own `location` field was deliberately *not* added as a trust source: it's the posting's onsite city, not necessarily HQ (remote/multi-office/branch postings routinely differ), and trusting it would let a same-name collision company's HQ pass just because it coincidentally matches the job's city. |

**Known, accepted limitation**: exact-token matching can still flag a *correct* headquarters value low-confidence on a bad enough abbreviation mismatch — e.g. `"Austin, TX, USA Headquarters Office"` (5 tokens) against official text that only spells out `"...Austin, Texas..."` scores 1/5 ≈0.2, below the 0.25 bar, even though it's right. Lowering the threshold narrowed this (a 3-token `"Austin, TX, USA"` now clears the bar at ≈0.33) but didn't eliminate it — longer values with only one matching token can still fall short. Not fixed further — official contact/about pages tend to give formatted addresses with abbreviations rather than prose, so this is judged rare; the value is still shown (just flagged, not hidden), so the cost of this limitation is lower than it was before the keep-not-discard change. Test-covered (`enrichment.processor.spec.ts`) as deliberate, not accidental.
...
| `FAILED` | false (never succeeded) | Header + Refresh button + a `FailureBanner` — pattern-matches `errorMessage` via `classifyFailure` into `RATE_LIMITED` / `UNAVAILABLE` / `CONFIG` with tailored copy+icon+tone, falling back to the raw `errorMessage` (plain red text, no box) if unrecognized |
| `FAILED` | true (re-run failed) | Full fields view + Refresh button + the same `FailureBanner`, with a `"Last refresh failed: "` prefix and `" — showing the last successful result."` suffix — the last successful data stays visible instead of being replaced by the error |
...
Two things are factored into shared components rather than duplicated across the four branches above:
- **`ProfileFields`** — the field-rendering JSX (grid/pills/address/etc), shared by the `COMPLETED` case and the `hasData` branches of `PENDING`/`PROCESSING`/`FAILED`.
- **`FailureBanner`** — the classify-and-render-a-failure logic (icon/tone/message lookup via `classifyFailure`/`FAILURE_COPY`, with an unclassified-message fallback), shared by both `FAILED` branches via optional `prefix`/`suffix` props. Before this was factored out, the `hasData` branch hardcoded a generic amber `WifiOff` banner with the raw `errorMessage` regardless of failure kind — a `CONFIG` (bad API key) failure on a re-run looked identical to a network blip instead of getting the red, `KeyRound`-iconed, "check `GROQ_API_KEY`" treatment the first-run case already gave it. `FailureBanner` closes that gap: both branches now classify identically.
...
Meanwhile, in the BullMQ worker:
  EnrichmentProcessor.process(job)
    ...
    → deterministic guard on address (0.7) and headquarters (0.25) — flags low-confidence, doesn't discard
    → CompanyProfile updated: status COMPLETED, all fields, enrichedAt
    → (if this final write itself fails: catch block salvages the already-extracted data as COMPLETED anyway — see §3/§10)
...
**Follow-up — lower `headquarters` guard threshold from 0.4 to 0.25** (`enrichment.processor.ts`)
- Decision: lower the `headquarters` token-overlap bar from ≥40% to ≥25%. `address` stays at 0.7 — untouched.
- Problem solved: fewer correct-but-low-overlap headquarters values get the "unverified" badge (e.g. the abbreviation-mismatch case in §6 — `"Austin, TX, USA"` now clears the bar at ≈0.33 instead of falling just short of the old 0.4). This was only viable once the guard stopped discarding below-bar values (the earlier change in this log) — loosening a threshold that used to overwrite to `"Unknown"` would have meant *more* wrong values reaching the user with no signal at all; loosening a threshold that now just toggles a badge only means fewer badges, correct or not.
- Side effects: more same-name-collision headquarters values will now clear the bar without a badge — i.e. more chances of a confidently-wrong HQ shown with no "unverified" marker at all, since 0.25 is a meaningfully lower bar for a same-name company operating in a different city to accidentally clear (e.g. shared country/state tokens). This is the direct trade against the stated priority: fewer false "unverified" flags on correct data, at the cost of some wrong data now looking equally confident. Not measured empirically in this pass — worth revisiting if collision-related bug reports increase. The known abbreviation-mismatch limitation (§6) is narrowed, not eliminated — a longer low-overlap value can still fall under 0.25 and get flagged.

**Follow-up — salvage already-extracted data on a late pipeline failure** (`enrichment.processor.ts`)
- Decision: track successful extraction+guard output in an outer-scoped `extraction` variable. If a step *after* that (the `stillExists` re-check or the `COMPLETED` write itself) throws, the `catch` block now attempts one salvage write of that data as `COMPLETED` instead of immediately falling back to a bare `FAILED` write. On salvage success, the function returns normally (no rethrow — no point in BullMQ retrying a pipeline that already produced good data). On salvage failure, it falls through to the original FAILED-write-and-rethrow behavior unchanged.
- Problem solved: the narrow case flagged when this option was first raised — search/fetch/LLM failures were already covered by BullMQ's own retry (`attempts: 2`), but a DB hiccup on the *final* write threw away a fully successful, already-paid-for extraction (one Tavily search + up to 5 fetches + one Groq call) for a transient error unrelated to the data's correctness.
- Side effects: the `catch` block now makes an extra Prisma call in this one scenario (the salvage attempt), and can return successfully from what looks like an exception path — worth knowing when reading the code, hence the inline comments. Doesn't help the more common total-failure case (Groq timeout, search down) where `extraction` was never set — that's unaffected, still goes through the original FAILED/rethrow path. Test-covered: `enrichment.processor.spec.ts`'s "salvaging already-extracted data on a late failure" block (salvage succeeds → resolves without rethrow; salvage also fails → rethrows the *original* error, not the salvage error; no `extraction` set → no salvage attempted at all).
- **Follow-up — `phase` field on `enrichment_profile_update_failed`**: this log event fires from one shared `catch (updateErr)` covering two different situations — a plain FAILED-status write failing, vs. a salvage write failing after data was already extracted (data actually lost, not just a status marker). Without a way to tell them apart, an operator reading logs couldn't distinguish "run failed, and we couldn't even record that" from the strictly worse "run succeeded, and we lost the result." Added `phase: 'salvage' | 'mark_failed'` to the log payload, set once (`extraction ? 'salvage' : 'mark_failed'`) before the inner `try`. No behavior change — logging only. Test-covered: both phase values asserted directly against `mockLogger.warn` calls.
- **Follow-up — extracted the catch-block logic and the write-payload shape** (post-review cleanup, no behavior change): the salvage-or-FAILED decision (including the `phase` logic above) moved into a private `recordFailureOutcome(jobId, company, extraction, errorMessage, startedAt)` method returning `boolean` (`true` only when a salvage write succeeds, telling `process()` not to rethrow) — `process()`'s `catch` block is now a few lines instead of ~50. Separately, the `COMPLETED`-write payload (`status`, spread extracted fields, both low-confidence flags, `enrichedAt`) was being built identically in two places — the happy path and the salvage path — a duplication that would've silently drifted if a field were ever added to one write but not the other; both now call a shared `private buildCompletedProfileData(extraction: ExtractionResult)`. `ExtractionResult` (the `{ data, lowConfidence }` shape) is now a named top-level type instead of an inline literal. All 31 processor tests (including a new one pinning the guard's `< 0.25` boundary — exactly `0.25` must be accepted, not flagged, guarding against an accidental `<=` flip) pass unchanged against the refactor, confirming it's behavior-preserving.
