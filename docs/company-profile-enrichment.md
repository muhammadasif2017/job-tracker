...
- Client constructed with **`timeout: 30_000`** (30s) — a hard upper bound per request, so a hung Groq call can't stall the BullMQ job indefinitely. This number is what makes the `lockDuration` arithmetic in §3 closed-form rather than open-ended: worst-case sequential network chain (general search + fetch batch + `/contact-us` fallback + domain-scoped fallback search) is ≈40s, plus this 30s Groq bound ≈70s normal case, or ≈100s if a retry fires (next bullet) — both comfortably inside the lock's ~45s renewal cadence regardless of total length.
- `createWithRetry` — one immediate retry if Groq returns a `tool_use_failed` 400 (a known generation-time glitch, not a bad request) — cheaper than falling through to a full BullMQ retry, which re-runs search/fetch too.
