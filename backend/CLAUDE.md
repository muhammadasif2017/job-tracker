...
| `REDIS_URL`              | No       | `redis://localhost:6379` | BullMQ connection for the enrichment, timeline-summary and notifications queues (queue adds fail fast on an outage, workers wait it out — ADR-046); also backs the short-lived OAuth exchange-code store in `AuthService` |
