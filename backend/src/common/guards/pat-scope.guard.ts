...
export class PatScopeGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext) {
    ...
    // unaffected by this guard. Fail-closed on *any* scope value, not just
    // the current PAT_SCOPE - a future restricted scope that's added
    // without updating this guard stays locked out by default instead of
