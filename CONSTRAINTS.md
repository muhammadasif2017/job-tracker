...
| E6 | floor: assertion-removed | `backend/src/modules/companies/companies.service.spec.ts` | The `mergeCompanies` and `findDuplicateSuggestions` describes moved verbatim into `company-dedup.service.spec.ts` when `CompanyDedupService` was split out of `CompaniesService`. `floor-guard` counts the assertion delta per file (`scripts/floor-guard.mjs`, `assertionDelta`), so a split reads as 33 removals even though the same assertions land in a new file in the same commit and the repo-wide total is unchanged. Verified by the test count: 628 before and after | @muhammadasif2017 | 2026-12-31 |
